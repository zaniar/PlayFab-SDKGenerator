...
exports.makeCombinedAPI = function (apis, sourceDir, apiOutputDir) {
    ...
    var indexTemplate = getCompiledTemplate(path.resolve(templateDir, "index.ts.ejs"));
    var apiTemplate = getCompiledTemplate(path.resolve(templateDir, "PlayFab_Api.ts.ejs"));
    var apiTypingTemplate = getCompiledTemplate(path.resolve(templateDir, "PlayFab_Models.ts.ejs"));
    ...
    for (var fIdx = 0; fIdx < destSubFolders.length; fIdx++) {
        ...
        for (var i = 0; i < apis.length; i++) {
            ...
            writeFile(path.resolve(eachOutputDir, "src/" + apis[i].name + "Api.ts"), apiTemplate(locals));
            if (i == 0) writeFile(path.resolve(eachOutputDir, "src/index.ts"), indexTemplate(locals));
            writeFile(path.resolve(eachOutputDir, "src/Models/" + apis[i].name + "Models.ts"), apiTypingTemplate(locals));
            if (destSubFolders[fIdx] !== "PlayFabTestingExample") {
                writeFile(path.resolve(eachOutputDir, "package.json"), packageTemplate(locals));
                writeFile(path.resolve(eachOutputDir + "/lib", "package.json"), packageTemplate(locals));
                writeFile(path.resolve(eachOutputDir, "tsconfig.json"), tsconfigTemplate(locals));
            }
        }
    }
...
function makeSimpleTemplates(apis, templateDir, apiOutputDir) {
    ...
    var coreTyping = getCompiledTemplate(path.resolve(templateDir, "Models.ts.ejs"));
    var genCoreTypings = coreTyping(apiLocals);
    writeFile(path.resolve(apiOutputDir, "src/Models/index.ts"), genCoreTypings);
}
...
function getRequestActions(tabbing, apiCall) {
    if (apiCall.url === "/Authentication/GetEntityToken")
        return tabbing + "var authKey = null; var authValue = null;\n"
            + tabbing + "if (!authKey && InternalSettings.sessionTicket) { var authInfo = InternalSettings.GetAuthInfo(request, \"X-Authorization\"); authKey = authInfo.authKey, authValue = authInfo.authValue; }\n"
            + tabbing + "if (!authKey && Settings.developerSecretKey) { var authInfo = InternalSettings.GetAuthInfo(request, \"X-SecretKey\"); authKey = authInfo.authKey, authValue = authInfo.authValue; }\n";
    if (apiCall.result === "LoginResult" || apiCall.request === "RegisterPlayFabUserRequest")
        return tabbing + "request.TitleId = Settings.titleId ? Settings.titleId : request.TitleId; if (!request.TitleId) throw InternalSettings.errorTitleId;\n"
            + tabbing + "// InternalSettings.authenticationContext can be modified by other asynchronous login attempts\n"
            + tabbing + "// Deep-copy the authenticationContext here to safely update it\n"
            + tabbing + "var authenticationContext = JSON.parse(JSON.stringify(InternalSettings.authenticationContext));\n";
    return "";
}
...
function getResultActions(tabbing, apiCall) {
    if (apiCall.result === "LoginResult")
        return tabbing + "if (result != null) {\n"
            + tabbing + "    if(result.data.SessionTicket != undefined) {\n"
            + tabbing + "        InternalSettings.sessionTicket = result.data.SessionTicket ?? null;\n"
            + tabbing + "    }\n"
            + tabbing + "    if (result.data.EntityToken != undefined) {\n"
            + tabbing + "        InternalSettings.entityToken = result.data.EntityToken.EntityToken ?? null;\n"
            + tabbing + "    }\n"
            + tabbing + "    // Apply the updates for the AuthenticationContext returned to the client\n"
            + tabbing + "    authenticationContext = InternalSettings.UpdateAuthenticationContext(authenticationContext, result);\n"
            + tabbing + "}";
    if (apiCall.result === "RegisterPlayFabUserResult")
        return tabbing + "if (result != null && result.data.SessionTicket != undefined) {\n"
            + tabbing + "    InternalSettings.sessionTicket = result.data.SessionTicket ?? null;\n"
            + tabbing + "}";
    if (apiCall.url === "/Authentication/GetEntityToken")
        return tabbing + "if (result != null && result.data.EntityToken != undefined)\n"
            + tabbing + "    InternalSettings.entityToken = result.data.EntityToken ?? null;";
    return "";
}
...
function getUrl(apiCall) {
    return "InternalSettings.GetServerUrl() + " + getUrlPath(apiCall);
}

function getAuthParams(apiCall) {
    // Returns the authKey to InternalSettings.ExecuteRequestWrapper()
    if (apiCall.url === "/Authentication/GetEntityToken")
        return "authKey";
